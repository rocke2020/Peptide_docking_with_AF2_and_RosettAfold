...
def calc_rmsd_by_pymol(pdb_file:str, pdb_id_chains:str, full_seq:str):
    """  
    Args:
        pdb_id_chains: 2fmf_AB
    """
    complex=pdb_id_chains
    ...
    native_rec_chain, native_pep_chain = list(splitted_complex[1])
    ...
    cmd.color('cyan', 'interface_pep')

    # cmd.show(representation='sticks', selection='native_pep')
    cmd.save(f'{analysis_out_dir}/{complex}_native.pse', format='pse')
    ...
    cmd.save(f'{analysis_out_dir}/{complex}.pse', format='pse')
    ...
    metrics_df.to_csv(analysis_out_dir / (complex + '.csv'), index=False)
...
def main():
    ...
    for i, row in fasta_df.iterrows():
        ...
        if not check_seq(merged_seq):
            ...
            continue
        try:
            calc_rmsd_by_pymol(pdb_file, pdb_id_chains, full_seq)
